...
namespace MEM
{
	...
	template <typename T = void*>
	inline T GetVMethod(void* class_, unsigned int index) {
		if (!class_)
			return T{};
	...
	inline T CallVirtual(void* class_, unsigned int index, Args... args) {
		auto func = GetVMethod<T(__thiscall*)(void*, Args...)>(class_, index);
