...
bool ImGui::HotKey(const char* szLabel, unsigned int* pValue)
{
	...
	if (*pValue != 0 && g.ActiveId != nIndex)
		szBufferEnd = CRT::StringCat(szBufferEnd, (*pValue < IM_ARRAYSIZE(arrKeyNames)) ? arrKeyNames[*pValue] : "?");
...
bool ImGui::ColorEdit4(const char* szLabel, ColorPickerVar_t* pColorVar, ImGuiColorEditFlags flags)
{
	...
	SetNextWindowSize(
		ImVec2(
			(pColorVar->bRainbow ? 120.f : 60.f) * 1.0f,
			0.f
		)
	);
	if (BeginPopup(CS_XOR("context##color.picker")))
	{
		if (Button(CS_XOR("copy##color.picker"), ImVec2(-1, 15 * 1.0f)))
		{
		...
		if (Button(CS_XOR("paste##color.picker"), ImVec2(-1, 15 * 1.0f)))
		{
...
#pragma endregion
	...
	{
		const float flFontSize = 12.f * 1.0f;
...
bool D::OnWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		gui::open = !gui::open;
		MENU::bMainWindowOpened = gui::open;
		// update animation
		...
		oIsRelativeMouseMode(I::InputSystem, gui::open ? false : MENU::bMainActive);
	}

	// handle ImGui's window messages and block game's input if menu is opened
	return ImGui_ImplWin32_WndProcHandler(hWnd, uMsg, wParam, lParam) || gui::open;
}
