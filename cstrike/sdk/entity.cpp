...
inline bool C_CSPlayerPawn::IsLocalPlayerPawn()
{
	CBaseHandle hController = GetControllerHandle();
	if (!hController.IsValid())
		return false;

	CCSPlayerController* pController = I::GameResourceService->pGameEntitySystem->Get<CCSPlayerController>(hController);
	if (!pController)
		return false;

	return pController->IsLocalPlayerController();
}
...
{
	return GetGameSceneNode()->GetAbsOrigin() + GetViewOffset();
}
C_CSWeaponBase* C_CSPlayerPawn::GetActiveWeapon()
{
	CPlayer_WeaponServices* pWeaponServices = GetWeaponServices();
	...
	return I::GameResourceService->pGameEntitySystem->Get<C_CSWeaponBase>(nIndex);
}
